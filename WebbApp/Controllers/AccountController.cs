...
public class AccountController(UserManager<UserEntity> userManager, ApplicationContext context) : Controller
{
    ...
    public async Task<IActionResult> UpdateBasicInfo(AccountDetailsViewModel model) 
    {
        ...
        if (TryValidateModel(model.Basic!)) 
        {
            ...
            { 
                ...
                user.Bio = model.Basic!.Biography;


                var result = await _userManager.UpdateAsync(user);
                if (result.Succeeded) 
                {
                    TempData["StatusMessage"] = "Basic information has been succefully updated.";
        ...
        if (TryValidateModel(model.Address!))
        {
            ...
            { 
                try 
                {
                    ...
                    TempData["StatusMessage"] = "Address information has been succefully updated.";
    ...
    public async Task<IActionResult> UploadProfileImage(IFormFile file) 
    {
        ...
        { 
            ...
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/images/account-profile", fileName);
